...
resource "databricks_sql_endpoint" "serverless" {
  # This loop tells Terraform to create one resource for every item in the tfvars map
  for_each = var.sql_warehouses

  # Dynamically assign names like: dev-bi-reporting-sql
  name                      = "${var.environment}-${each.key}-sql"
  
  # Pull the specific settings from the tfvars file
